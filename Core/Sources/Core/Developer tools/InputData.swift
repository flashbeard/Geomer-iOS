...
//	taskRegistry.add(instances: taskSSS, taskSAS, taskASA)
	...
	let ac = Segment(a: a, b: c, length: LinearUnit.new(value: 15))

	let abd = Triangle(vertexes: [a, b, d])
	let acd = Triangle(vertexes: [a, c, d])

	let sideEq = BEEquality(left: ab, right: ac)

	let ang1 = Angle(a: b, b: a, c: d)
	let ang2 = Angle(a: c, b: a, c: d)
	let angEq = BEEquality(left: ang1, right: ang2)

	nodeRegistry.add(instances: abd, acd, sideEq, angEq)

	let task = Task(task: BEPolygonEquality(left: abd, right: acd, leftShift: Shift()))
