	...
	internal override func execute() {
		...
		for i in 0...2 {
			for reversed in [true, false] {
				...
				if equalityAnglesA && ratioSidesCA == ratioSidesAB {
					result.append(BEPolygonSimilarity(left: shifted, right: triangle2, leftShift: Shift(i, reversed: reversed), ratio: ratioSidesAB))
				} else if equalityAnglesB && ratioSidesAB == ratioSidesBC {
					result.append(BEPolygonSimilarity(left: shifted, right: triangle2, leftShift: Shift(i, reversed: reversed), ratio: ratioSidesBC))
				} else if equalityAnglesC && ratioSidesBC == ratioSidesCA {
					result.append(BEPolygonSimilarity(left: shifted, right: triangle2, leftShift: Shift(i, reversed: reversed), ratio: ratioSidesCA))
				}
