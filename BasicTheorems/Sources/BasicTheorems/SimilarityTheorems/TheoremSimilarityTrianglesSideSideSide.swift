...
internal class TheoremSimilarityTrianglesSideSideSide: Theorem {
	...
	internal override func execute() {
		...
		for i in 0...2 {
			...
			if ((shifted.side(.AB).value?.value / shifted.side(.BC).value).fraction() == (triangle2.side(.AB).value / triangle2.side(.BC).value).fraction()) && (shifted.side(.BC).value / shifted.side(.CA).value).fraction() == (triangle2.side(.BC).value / triangle2.side(.CA).value).fraction() {
